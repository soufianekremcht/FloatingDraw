...
class MyCanvas(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    var mPaths = LinkedHashMap<CustomPath, PaintOptions>()
    var mBackgroundBitmap: Bitmap? = null
    var mListener: CanvasListener? = null

    private var mLastPaths = LinkedHashMap<CustomPath, PaintOptions>()
    private var mUndonePaths = LinkedHashMap<CustomPath, PaintOptions>()
    private var mLastBackgroundBitmap: Bitmap? = null

    private var mPaint = Paint()
    private var mPath = CustomPath()
    ...
    fun undo() {
        if (mPaths.isEmpty() && mLastPaths.isNotEmpty()) {
            mPaths = mLastPaths.clone() as LinkedHashMap<CustomPath, PaintOptions>
    ...
    fun drawBitmap(activity: BaseActivity, path: Any) {
        Thread {
            val size = Point()
    ...
    fun addPath(path: CustomPath, options: PaintOptions) {
        mPaths[path] = options
        pathsUpdated()
    }
    ...
    fun clearCanvas() {
        mLastPaths = mPaths.clone() as LinkedHashMap<CustomPath, PaintOptions>
